...
export default function AvailableTrainsPage() {
  ...
  const fetchTrains = async (from, to, date) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5001/api/trains/search?from=${from}&to=${to}${dateQuery}`,
      );
      if (response.ok) {
        const data = await response.json();
        setTrains(data);
        ...
        setError(
          "Unable to fetch train schedules for the selected date/station combination.",
        );
      }
    } catch (err) {
  ...
  const handleToStationSearch = (value) => {
    setToStation(value);
            ...
            <div className="relative">
              ...
              <div className="relative">
                <input
                  ...
                  onBlur={() =>
                    setTimeout(() => setShowFromDropdown(false), 150)
                  }
                  ...
                  {filteredFromStations.map((station) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-400">
                        {station.station_code} • {station.city}, {station.state}
                      </div>
                    </button>
                  ))}
                </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 mb-4">
              Try different dates or stations
            </p>
          </div>
        ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {new Date(train.travel_date).toLocaleDateString()}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
